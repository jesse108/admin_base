...
class DB{
	const DB_TYPE_RW = 'rw';
	const DB_TYPE_RO = 'ro';
	const DB_TYPE_INTERFACE = 'interface';
	const DB_TYPE_TEST = 'test';
    ...
    private static $_trans_enabled = false;
	...
	static public function LimitQuery ($table,$condition = array(),$options = array(),$dbType = self::DB_TYPE_RO) {
		...
		return self:: GetQueryResult ( $sql, $one,$dbType);
	}
	...
	public static function Exists($table,$condition,$column = 'id',$dbType = null){
		...
		$sql .= ' limit 1 ';
		$result = self::GetQueryResult($sql,true,$dbType);
		if($result){
			return true;
		} else {
			return false;
	...
	static public function TransBegin() {
        $dbObject = self::_GetDBObject(self::DB_TYPE_RW);
        $ret = $dbObject->transBegin();
        self::$_trans_enabled = true;
        return $ret;
    }

    static public function TransCommit(){
        $dbObject = self::_GetDBObject(self::DB_TYPE_RW);
        $ret = $dbObject->transCommit();
        self::$_trans_enabled = false;
        return $ret;
    }

    static public function TransRollback(){
        $dbObject = self::_GetDBObject(self::DB_TYPE_RW);
        $ret = $dbObject->transRollback();
        self::$_trans_enabled = false;
        return $ret;
    }
    ...
    static public function TransStatus(){
        $dbObject = self::_GetDBObject(self::DB_TYPE_RW);
        $ret = $dbObject->transStatus();
        return $ret;
    }

    static public function TransComplete(){
        $ret = true;
        if(self::$_trans_enabled){
            $dbObject = self::_GetDBObject(self::DB_TYPE_RW);
            if($dbObject->transStatus()){
                $ret = $dbObject->transCommit();
            }else{
                $ret = $dbObject->transRollback();
            }
	...
	private static function _GetDBObject($dbType = null){
        ...
        if(self::$_trans_enabled){  	
        	$dbType = $dbType == self::DB_TYPE_RO ?  self::DB_TYPE_RW : $dbType;
        }
    ...
    public static function SQLDebug($debug = null){
        if($debug === null){
            $debug = self::$_debug ? false : true;
        }
        $debug = $debug ? 'footer' : false;
        foreach (self::$_dbObjectList as $dbObject){
            $dbObject->debug = $debug;
        }
        self::$_debug = $debug;
...
class DBObject{
	...
	function __construct($dbConfig){
		...
		$this->_connection = @mysql_connect($host,$user,$password,true);
	...
	function query($sql){
		...
		$this->count++;
		if($this->debug){
			Util_Time::timerStart($sqlMD5);
		}
		$result = @mysql_query($sql,$this->_connection);
		if($this->debug){
            $duration = Util_Time::timerStop($sqlMD5,Util_Time::TIMER_PRECISION_MSEC);
            $rowNum = mysql_affected_rows($this->_connection);
            $count = $this->count;

            if($this->debug === 'footer'){
                DB_Stat::push(array(
                    'total' => $count,
                    'row'   => $rowNum,
                    'time'  => $duration,
                ...
                $statRecord = "[{$count}][ROW:{$rowNum}][time:{$duration} ms]{$sql}";
                echo "<pre>{$statRecord}</pre>\r\n";
            }
		...
		if($result){
            ...
            if($this->_trans_depth > 0){$this->_trans_status = false;
