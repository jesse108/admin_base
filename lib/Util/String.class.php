...
class Util_String{
	...
	public static function ParseString($keywords, $default=''){
		if(!$keywords) return $default;
		
		$field= '';
		if(Util_Validator::checkIdCard($keywords)){ // 身份证
			$field = 'id_card';
	...
	public static function ParseIdCard($id_card){
		if(!$id_card) return array();
        $id_card = strtoupper($id_card);
		
		$brithday = substr($id_card, 6, 8);
		$brithday = date('Y-m-d', strtotime($brithday)); // 生日
        $brithday = $brithday ? strtotime($brithday) : 0;

        $age      = date('Y') - date('Y', $brithday);
        if(date('Y') . date('-m-d', $brithday) < date('Y-m-d')){
            $age  = $age - 1;
        }

        $sex	  = substr($id_card, 16, 1); //  if(substr(o.id_number, 17, 1)%2=0,'女', '男') as 性别,
		$sex      = ($sex%2==1) ? Config_Company_User::SEX_MAN : Config_Company_User::SEX_WOMAN;

		$area	  = substr($id_card, 0, 6);
		...
		return array(
            ...
            'id_card'   => $id_card,
            'brithday' 	=> $brithday,
            'sex_title' => isset(self::$sexTitle[$sex]) ? self::$sexTitle[$sex] : '',
            'brithday_date' => date('Y-m-d', $brithday),
		);
