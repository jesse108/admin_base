...
class Util_Upload {
    ...
    public function __construct($config = array())
    {
        ...
        $this->security = new Util_Security();
    ...
    public function data($index = NULL)
    {
        $data = array(
            ...
            'file_path'		=> $this->images_path,
            'full_path'		=> $this->images_path.$this->file_name,
    ...
    public function validate_upload_path()
    {
        ...
        if(!file_exists($this->upload_path)){
            $result = mkdir($this->upload_path, 0777, true);
            if(!$result){
                $this->set_error('upload_path_mkdir_failed');
                return FALSE;
            }
        }
    ...
    {
        $lang = Config::Get('upload', 'lang');

        is_array($msg) OR $msg = array($msg);

        foreach ($msg as $val) {
            $msg = (isset($lang[$val]) && $lang[$val]) ? $lang[$val] : $val;
            $this->error_msg[] = $msg;
        }
    ...
    {
        $display_errors = (count($this->error_msg) > 0) ? $open.implode($close.$open, $this->error_msg).$close : '';
        $display_errors = $display_errors ? trim($display_errors) : '';
    ...
    protected function _file_mime_type($file)
    {
        ...
        if (DIRECTORY_SEPARATOR !== '\\')
        {
            ...
            if (function_exists('function_usable') && function_usable('exec'))
            {
            ...
            if ( ! ini_get('safe_mode') && function_exists('function_usable') && function_usable('shell_exec'))
            {
                $mime = @shell_exec($cmd);
                if (strlen($mime) > 0)
                {
            ...
            if (function_exists('function_usable') && function_usable('popen'))
            {
                $proc = @popen($cmd, 'r');
                if (is_resource($proc))
                {
